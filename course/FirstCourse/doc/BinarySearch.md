...
![image-20210811003133204](https://raw.githubusercontent.com/chihokyo/image_host/master/20210814020118.png)
...
因为具有单调性，就是假设 `piles = [3,6,7,11], H = 8` 每小时吃的越多，总时间越长。

每小时3根， 1+ 2+ 3 + 4 = 10 每小时4根：1 + 2 + 2 + 3 = 8 

这样具有单调性， `k↑ -> eatingTime(k) ↓`，二分搜索 `eatingTIme(k) <= H`的最大eatingTIme对应的k

**Q：思路的难点？**

就是你怎么想得到有这种单调性，这个k也是变量的情况下，很难去思考到，平常得出的结果就行。这一次需要多一层函数。

能理解出来用二分法也很难，其实直接用暴力解法，就是从1开始到MAX最大值，一个个计算。
...
![image-20210814020046052](https://raw.githubusercontent.com/chihokyo/image_host/master/20210814020047.png)

就是，days我不知道怎么写？下面是一个完整的解
...
最难理解的应该是为什么最后days，需要在res++一次，我的理解就是即使你一次性全部拉走，最后也是需要1天时间的，所以这个1是无论如何也要加上的。
